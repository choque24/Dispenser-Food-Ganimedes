...
const int LED = 23;
const int VUELTAS_POR_PORCION = 20;
const int DELAY_REBOTE = 50;
const int MAX_PORCIONES = 10;
const int PORCIONES_AUTO = 2;  // Porciones para dispensación automática

// Variables para el tiempo
ESP32Time rtc;
unsigned long ultimaDispensacion = 0;
const unsigned long INTERVALO_DISPENSACION = 12 * 3600000;  // 12 horas en milisegundos
...
void dispensarComida() {
  ...
  ultimaDispensacion = millis();  // Actualizar el tiempo de última dispensación
  porciones = 0;
...
void dispensarComidaAutomatica() {
  porciones = PORCIONES_AUTO;
  dispensarComida();
  ultimaDispensacion = millis();  // Actualizar el tiempo de última dispensación
}

void verificarHoraDispensacion() {
  if (millis() - ultimaDispensacion >= INTERVALO_DISPENSACION) {
